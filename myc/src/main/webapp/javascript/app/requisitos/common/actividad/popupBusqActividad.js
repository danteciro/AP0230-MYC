...
function buscarTreeData(){
    $.ajax({
        ...
        async: false,
        data: {
            idProcedimiento:$('#idProcedimientoAct').val(),
            idTramite:$('#idTramiteAct').val()
        },
...
$('#btnSeleccionarAct').click(function() {
    //document.getElementById("txtActivP1").value = document.getElementById("idActividadesSelecEspe").value;
    $("#txtActivP1").val('');
    ...
    if(document.getElementById("idActividadesSelecEspe").value == '' || document.getElementById("idActividadesSelecEspe").value == '[]'){
        ...
        $("#txtActivP1").val( $.map(data,function(item){return item.nombre;}).join(', ') );
        $("#txtActivP1").attr('title', $.map(data,function(item){return item.nombre;}).join(', ') );
        $("#txtIdActivP1").val( $.map(data,function(item){return item.id;}).join(',') );
    }
